...
void matran_qh_dich_mau_do(usi8 j){                            
   xoa_dl_48_cot();      
   sapxep_vitri_48byte_cot_do(j);                          
   hien_thi_ma_tran_quet_hang(50);                    
}                                        
void matran_qh_dich_mau_cam(usi8 k){                            
   xoa_dl_48_cot();                     
   sapxep_vitri_48byte_cot_do(k); 
   sapxep_vitri_48byte_cot_xanh(k);                  
   hien_thi_ma_tran_quet_hang(50);                    
}
void main() {                                                            
   set_up_port();                                                                          
   ...
   while(true){  
         ...
         matran_qh_dich_mau_cam(vt);        
         }                                  
         for(vt=0;vt<(slkt-4)*6;vt++)     {
         matran_qh_dich_mau_xanh(vt);  
         }   
         for(vt=0;vt<(slkt-4)*6;vt++)     {
         matran_qh_dich_mau_do(vt);  
         }  
